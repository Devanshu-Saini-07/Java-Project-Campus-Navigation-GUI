...
public class CampusDataManager {
    ...
    private void createDefaultLocationsFile() {
        ...
        // ID | Name | Category | Building | Floor | Description | Nearby | Timings | X | Y | Aliases
    ...
    private void createDefaultEdgesFile() {
        ...
        // SourceID | DestID | Distance (meters)
        defaultEdges.add(new CampusEdge("LOC01", "LOC21", 50));   // Main Gate <-> Main Bus Stop
        defaultEdges.add(new CampusEdge("LOC01", "LOC02", 120));  // Main Gate <-> Admin Block
        defaultEdges.add(new CampusEdge("LOC21", "LOC02", 90));   // Main Bus Stop <-> Admin Block
        defaultEdges.add(new CampusEdge("LOC02", "LOC06", 110));  // Admin Block <-> Auditorium
        defaultEdges.add(new CampusEdge("LOC02", "LOC03", 150));  // Admin Block <-> Academic Block A
        defaultEdges.add(new CampusEdge("LOC06", "LOC22", 100));  // Auditorium <-> Library Bus Stop
        defaultEdges.add(new CampusEdge("LOC22", "LOC05", 60));   // Library Bus Stop <-> Library
        defaultEdges.add(new CampusEdge("LOC05", "LOC03", 110));  // Library <-> Academic Block A
        defaultEdges.add(new CampusEdge("LOC05", "LOC14", 80));   // Library <-> AI Lab
        defaultEdges.add(new CampusEdge("LOC03", "LOC07", 30));   // Acad Block A <-> CSE Dept
        defaultEdges.add(new CampusEdge("LOC03", "LOC08", 40));   // Acad Block A <-> IT Dept
        defaultEdges.add(new CampusEdge("LOC03", "LOC11", 30));   // Acad Block A <-> ECE Dept
        defaultEdges.add(new CampusEdge("LOC07", "LOC12", 20));   // CSE Dept <-> Programming Lab
        defaultEdges.add(new CampusEdge("LOC07", "LOC14", 50));   // CSE Dept <-> AI Lab
        defaultEdges.add(new CampusEdge("LOC08", "LOC13", 25));   // IT Dept <-> Networking Lab
        defaultEdges.add(new CampusEdge("LOC11", "LOC15", 25));   // ECE Dept <-> Electronics Lab
        defaultEdges.add(new CampusEdge("LOC03", "LOC04", 140));  // Acad Block A <-> Acad Block B
        defaultEdges.add(new CampusEdge("LOC04", "LOC09", 30));   // Acad Block B <-> Mech Dept
        defaultEdges.add(new CampusEdge("LOC04", "LOC10", 30));   // Acad Block B <-> Civil Dept
        defaultEdges.add(new CampusEdge("LOC04", "LOC20", 130));  // Acad Block B <-> Sports Complex
        defaultEdges.add(new CampusEdge("LOC01", "LOC20", 350));  // Main Gate <-> Sports Complex
        defaultEdges.add(new CampusEdge("LOC20", "LOC23", 100));  // Sports Complex <-> Hostel Bus Stop
        defaultEdges.add(new CampusEdge("LOC23", "LOC19", 60));   // Hostel Bus Stop <-> Guest House
        defaultEdges.add(new CampusEdge("LOC19", "LOC18", 60));   // Guest House <-> Girls Hostel
        defaultEdges.add(new CampusEdge("LOC18", "LOC17", 70));   // Girls Hostel <-> Boys Hostel B
        defaultEdges.add(new CampusEdge("LOC17", "LOC16", 60));   // Boys Hostel B <-> Boys Hostel A
        defaultEdges.add(new CampusEdge("LOC05", "LOC16", 240));  // Library <-> Boys Hostel A
        defaultEdges.add(new CampusEdge("LOC04", "LOC18", 180));  // Acad Block B <-> Girls Hostel
    ...
    private void createDefaultRoomsFile() {
        ...
        // RoomNumber | Building | Floor | DepartmentOrLab | Description
    ...
    private void createDefaultBusFile() {
        ...
        // StopName | RouteNumber | Destination | Timings | NearbyLocations
    ...
    /**
     * Smart search supporting partial matches, case-insensitivity, category matches, and aliases.
     */
    public List<CampusLocation> searchLocations(String query) {
        ...
        // Check for exact alias mappings first (e.g. lib -> Library, cse -> Computer Science, hostel -> Hostel, admin -> Admin Block)
